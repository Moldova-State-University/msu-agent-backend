...
using MSUAgent.MobileBff.Endpoints;

namespace MSUAgent.MobileBff.Extensions;

public static class EndpointRouteBuilderExtensions
{
    public static void MapEndpointGroups(
        ...
        Assembly assembly)
    {
        ...
        {
            var attribute =
                type.GetCustomAttribute<ApiEndpointGroupAttribute>();

            if (attribute == null)
                continue;

            var instance =
                (IEndpointGroup)Activator.CreateInstance(type)!;

            var groupBuilder =
                builder.MapGroup(attribute.Route);
